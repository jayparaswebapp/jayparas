...
export function DestructiveActions({ userId, isDeleted }: { userId: string; isDeleted: boolean }) {
  const tCommon = useTranslations('common');
  const [confirming, setConfirming] = useState(false);
  const action = isDeleted ? restoreAppUserAction : softDeleteAppUserAction;
  ...
  if (!confirming) {
    return (
      ...
      >
        {isDeleted ? tCommon('actions.restore') : tCommon('actions.deactivate')}
      </button>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <SubmitButton
          label={isDeleted ? tCommon('actions.restore') : tCommon('actions.deactivate')}
