...
export function SupplierForm({ initial }: { initial: SupplierFormValues | null }) {
  const t = useTranslations('purchases.suppliers.form');
  const tCommon = useTranslations('common.actions');
  const [state, formAction] = useFormState<ActionResult | null, FormData>(saveSupplierAction, null);
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/purchases/suppliers" className="btn-ghost border border-neutral-300">
          {tCommon('cancel')}
        </Link>
      </div>
    </form>
