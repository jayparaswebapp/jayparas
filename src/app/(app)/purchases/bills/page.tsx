...
interface Row {
  ...
  bill_date: string;
  grand_total: number;
  supplier: { full_name: string; business_name: string | null } | null;
}
...
export default async function BillsListPage({
  ...
  let query = supabase
    .from('purchase_bills')
    .select(
      'id, bill_number, supplier_bill_number, business_line, status, bill_date, grand_total, supplier:suppliers(full_name, business_name)',
    )
    .is('deleted_at', null)
  ...
  if (q.length > 0) {
    ...
    query = query.or(`bill_number.ilike.${like},supplier_bill_number.ilike.${like}`);
  }

  const { data: rows } = await query;
  const bills = (rows ?? []) as unknown as Row[];
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          canWrite ? (
            <Link href="/purchases/bills/new" className="btn-primary !w-auto px-4">
              {t('newButton')}
            </Link>
          ) : null
        }
      ...
      >
        ...
        <select name="status" defaultValue={filters.status} className="input-base">
          ...
          <option value="issued">{t('statusPosted')}</option>
        ...
        <div className="flex items-center gap-2 sm:col-span-6">
          ...
          {hasFilter ? (
            <Link href="/purchases/bills" className="btn-ghost border border-neutral-300 text-sm">
              {t('clearButton')}
            </Link>
          ) : null}
        </div>
      ...
      <ul className="divide-y divide-neutral-200 overflow-hidden rounded-lg border border-neutral-200 bg-white">
        {rows.map((row) => {
          const dateStr = new Intl.DateTimeFormat(locale === 'gu' ? 'gu-IN' : 'en-IN', {
            ...
            month: 'short',
            year: 'numeric',
          }).format(new Date(row.bill_date));
          const supplierLabel = row.supplier
            ? row.supplier.business_name
              ? `${row.supplier.business_name} (${row.supplier.full_name})`
          ...
          return (
            <li key={row.id}>
              ...
              >
                ...
                <div className="mt-0.5 text-sm text-neutral-700">{supplierLabel}</div>
                <div className="text-xs text-neutral-500">
                  {dateStr}
                  {row.supplier_bill_number ? ` · ${row.supplier_bill_number}` : ''}
                </div>
              </Link>
            </li>
          );
